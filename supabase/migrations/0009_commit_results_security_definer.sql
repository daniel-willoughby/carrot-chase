-- =============================================================================
-- 0009_commit_results_security_definer.sql
--
-- commit_event_results was written without `security definer`, so when a
-- Lead invokes it (via the run-event flow or the offline replayer) the
-- INSERT into public.results is blocked by the result_*_admin RLS policies
-- — Leads only have SELECT.
--
-- Earlier "successful" runs from a Lead session were coincidentally being
-- exercised from the service-role smoke tests, which bypassed RLS. As soon
-- as the offline replayer fired it from an authenticated Lead client, the
-- 42501 surfaced.
--
-- Re-declare the function with `security definer` so it runs as its owner
-- (postgres) and can perform the housekeeping writes it needs. Authorisation
-- is now enforced in-function: the caller must lead the event's group, OR
-- be a school/super admin.
-- =============================================================================
...
declare
  ...
  v_group_id uuid;
begin
  -- Lookup course distance + group for the event.
  select c.distance_metres, e.group_id
    into v_course_distance, v_group_id
  ...
  -- Authorisation: caller must lead this group, or be a school/super admin.
  if not (
    exists (
